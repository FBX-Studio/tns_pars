...
```bash
# Полный сбор (включая Дзен через Selenium - обход капчи)
python final_collection.py --zen-selenium

# Только Яндекс.Дзен через Selenium
python collect_zen_selenium.py

# Быстрый сбор (VK + Новости)
python collect_safe_sources.py
```

**📖 См. QUICK_START_FINAL.md и SELENIUM_INTEGRATION.md для подробных инструкций.**
...
- **Автоматический сбор отзывов** из:
  ...
  - Яндекс Дзен и новостные сайты (web scraping + комментарии)
