...
# Usage: 
# curl.exe -b 'session=cookie_value' -fsSL --ssl-no-revoke https://app.factory.ai/cli/windows -o install.ps1
...
& curl.exe -fsSL --ssl-no-revoke -o $binaryPath $url
if ($LASTEXITCODE -ne 0) {
    Write-Error "Download failed"
}

Write-Info "Fetching and verifying checksum"
$expectedHash = (& curl.exe -fsSL --ssl-no-revoke $shaUrl).Trim()
if ($LASTEXITCODE -ne 0) {
...
& curl.exe --ssl-no-revoke -fsSL -o $rgBinaryPath $rgUrl
if ($LASTEXITCODE -ne 0) {
    Write-Error "Ripgrep download failed"
}

Write-Info "Fetching and verifying ripgrep checksum"
$rgExpectedHash = (& curl.exe --ssl-no-revoke -fsSL $rgShaUrl).Trim()
if ($LASTEXITCODE -ne 0) {
